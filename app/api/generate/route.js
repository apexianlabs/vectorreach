...
export async function POST(request) {
  try {
    ...
    // Check usage limits
    if (userId) {
      const usageRes = await fetch(`${process.env.DB_API_URL}/usage/check?user_id=${userId}&product=vectorreach`, {
        headers: { 'Authorization': `Bearer ${process.env.DB_API_KEY_VECTORREACH}` }
      })
    ...
    if (userId) {
      fetch(`${process.env.DB_API_URL}/usage/track`, {
        ...
        body: JSON.stringify({ user_id: userId, product: 'vectorreach', action: 'generate' })
      }).catch(() => {})
    }
