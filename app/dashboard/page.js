...
useEffect(() => {
  ...
  const handleLogout = () => {
    document.cookie = 'vec_token=; path=/; max-age=0'
    document.cookie = 'vec_user=; path=/; max-age=0'
    router.push('/login')
  }
    ...
    <div style={{width:220,background:'#0f172a',display:'flex',flexDirection:'column',minHeight:'100vh',flexShrink:0,position:'relative'}}>
          ...
          <div style={{background:'#fff',border:'1px solid #e2e8f0',borderRadius:12,padding:16,marginBottom:16}}>
            ...
            {usageData.used >= 2 && (
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                ...
                <a href="/billing" style={{fontSize:11,fontWeight:700,color:'#059669',textDecoration:'none'}}>Upgrade →</a>
              </div>
            )}
          </div>
        )}
      ...
      <div style={{padding:'20px 16px 16px',borderBottom:'1px solid rgba(255,255,255,0.06)'}}>
        <Link href="/" style={{display:'flex',alignItems:'center',gap:9,textDecoration:'none'}}>
          <div style={{width:28,height:28,borderRadius:7,background:'#059669',display:'flex',alignItems:'center',justifyContent:'center',fontSize:13,fontWeight:800,color:'#fff',flexShrink:0}}>V</div>
          <div>
            <p style={{fontSize:13,fontWeight:800,color:'#fff',lineHeight:1,letterSpacing:'-0.3px'}}>VectorReach</p>
            <p style={{fontSize:10,color:'#475569',lineHeight:1,marginTop:2}}>Fill in the prospect</p>
          </div>
        </Link>
      </div>
      ...
      <div style={{padding:'12px 12px 8px'}}>
        <Link href="/generate" style={{display:'flex',alignItems:'center',justifyContent:'center',gap:7,background:'#059669',color:'#fff',textDecoration:'none',borderRadius:8,padding:'10px',fontSize:13,fontWeight:600,letterSpacing:'-0.2px'}}>
      ...
      <div style={{padding:'12px',borderTop:'1px solid rgba(255,255,255,0.06)'}}>
        <div style={{background:'rgba(255,255,255,0.04)',borderRadius:10,padding:'10px 12px',marginBottom:8,border:'1px solid rgba(255,255,255,0.06)'}}>
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:6}}>
            ...
            <Link href="/billing" style={{fontSize:10,color:'#059669',textDecoration:'none',fontWeight:600}}>Upgrade</Link>
          </div>
          <div style={{background:'rgba(255,255,255,0.08)',borderRadius:3,height:3,marginBottom:6}}>
            <div style={{background:'#059669',borderRadius:3,height:3,width:`${Math.min(100,(items.length/3)*100)}%`,transition:'width 0.3s'}}/>
          </div>
        ...
        <button onClick={handleLogout}
          ...
          <div style={{width:28,height:28,borderRadius:7,background:'#059669',display:'flex',alignItems:'center',justifyContent:'center',fontSize:11,fontWeight:700,color:'#fff',flexShrink:0}}>
            {getInitials(user)}
          </div>
  ...
  return (
    <div style={{display:'flex',minHeight:'100vh',fontFamily:'Inter,system-ui,sans-serif',background:'#f8fafc'}}>
      ...
      <div style={{flex:1,display:'flex',flexDirection:'column',minWidth:0,overflow:'hidden'}}>
        ...
        <div style={{background:'#fff',borderBottom:'1px solid #f1f5f9',height:56,display:'flex',alignItems:'center',padding:'0 20px',gap:12,flexShrink:0}}>
          ...
          <div style={{flex:1}}>
            ...
            <p style={{fontSize:11,color:'#94a3b8',marginTop:1}}>Here's your VectorReach overview.</p>
          </div>
          <Link href="/generate"
            style={{background:'#059669',color:'#fff',padding:'7px 16px',borderRadius:8,fontSize:13,fontWeight:600,textDecoration:'none',letterSpacing:'-0.2px',flexShrink:0}}>
            + New
          </Link>
        </div>
        ...
        <div style={{flex:1,padding:'20px',overflowY:'auto'}}>
          ...
          <div style={{display:'grid',gridTemplateColumns: isMobile ? 'repeat(2,1fr)' : 'repeat(4,1fr)',gap:12,marginBottom:20}}>
            {[
              { label:'Total', value: items.length, sub: 'All time', color:'#059669', icon:'📄' },
        ...
        {usageData && usageData.plan === 'free' && (
          <div style={{background:'#fff',border:'1px solid #e2e8f0',borderRadius:12,padding:16,marginBottom:16}}>
            ...
            <div style={{background:'#f1f5f9',borderRadius:6,height:6,marginBottom:8}}>
              <div style={{background:usageData.used >= usageData.limit ? '#dc2626' : '#059669',borderRadius:6,height:6,width:Math.max(4,Math.min(100,(usageData.used/usageData.limit)*100))+'%',transition:'width 0.3s'}}/>  
            </div>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
              <span style={{fontSize:11,color:'#94a3b8'}}>Resets {new Date(usageData.reset_date).toLocaleDateString('en-US',{month:'short',day:'numeric'})}</span>
              <a href="/billing" style={{fontSize:11,fontWeight:700,color:'#059669',textDecoration:'none'}}>Upgrade →</a>
            ...
            <div style={{background:'#fff',borderRadius:14,border:'1px solid #f1f5f9',overflow:'hidden',boxShadow:'0 1px 3px rgba(0,0,0,0.04)'}}>
              <div style={{padding:'16px 20px',borderBottom:'1px solid #f8fafc',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
                ...
                <Link href="/generate" style={{fontSize:12,color:'#059669',textDecoration:'none',fontWeight:600,background:'rgba(0,0,0,0.03)',padding:'4px 10px',borderRadius:6}}>+ New</Link>
              </div>
              ...
              {loading ? (
                ...
                <div style={{padding:'48px 24px',textAlign:'center'}}>
                  ...
                  <Link href="/generate" style={{background:'#059669',color:'#fff',padding:'9px 20px',borderRadius:8,fontSize:13,fontWeight:600,textDecoration:'none'}}>
                    Get started →
                  </Link>
                </div>
                ...
                <div key={item.id}
                  ...
                  <div style={{display:'flex',alignItems:'center',gap:12,minWidth:0,flex:1}}>
                    <div style={{width:34,height:34,borderRadius:9,background:'#f0f4ff',display:'flex',alignItems:'center',justifyContent:'center',fontSize:13,fontWeight:700,color:'#059669',flexShrink:0,border:'1px solid rgba(0,0,0,0.04)'}}>
                  ...
                  <Link href={`/items/${item.id}`}
                    style={{color:'#059669',fontSize:11,fontWeight:600,textDecoration:'none',padding:'5px 10px',border:'1px solid rgba(0,0,0,0.08)',borderRadius:6,flexShrink:0,marginLeft:12,background:'#fff'}}>
                    View →
                  </Link>
                </div>
              ))}
            ...
            <div style={{display:'flex',flexDirection:'column',gap:12}}>
              ...
              {/* Quick action */}
              <div style={{background:'linear-gradient(135deg,#059669 0%,#059669cc 100%)',borderRadius:14,padding:20,color:'#fff'}}>
                <p style={{fontSize:13,fontWeight:700,marginBottom:4,letterSpacing:'-0.2px'}}>Ready to generate?</p>
                <p style={{fontSize:12,color:'rgba(255,255,255,0.75)',marginBottom:14,lineHeight:1.5}}>Fill in the prospect details and get a personalised, research-backed cold outreach message instantly.</p>
              ...
              <div style={{background:'#fff',borderRadius:14,border:'1px solid #f1f5f9',padding:20,flex:1,boxShadow:'0 1px 3px rgba(0,0,0,0.04)'}}>
                ...
                {items.length === 0 ? (
                  ...
                  <div key={i} style={{display:'flex',alignItems:'flex-start',gap:10,marginBottom:12}}>
                    <div style={{width:26,height:26,borderRadius:'50%',background:'#f0f4ff',display:'flex',alignItems:'center',justifyContent:'center',fontSize:11,flexShrink:0,color:'#059669',fontWeight:700,border:'1px solid rgba(0,0,0,0.04)'}}>+</div>
                    <div style={{minWidth:0}}>
              ...
              {items.length >= 2 && (
                <div style={{background:'#fafafa',borderRadius:14,border:'1px solid #f1f5f9',padding:20}}>
                  ...
                  <Link href="/billing" style={{display:'block',textAlign:'center',background:'#059669',color:'#fff',textDecoration:'none',borderRadius:8,padding:'8px',fontSize:12,fontWeight:700}}>
                    View plans →
                  </Link>
                </div>
              )}
      ...
      <div style={{borderTop:'1px solid #e2e8f0',padding:'14px 28px',display:'flex',alignItems:'center',justifyContent:'space-between',background:'#fff',marginTop:'auto'}}>
        <span style={{fontSize:11,color:'#94a3b8'}}>© 2026 VectorReach</span>
        <div style={{display:'flex',gap:20}}>
          <a href="https://vectorreach.app/privacy" style={{fontSize:11,color:'#94a3b8',textDecoration:'none'}} target="_blank" rel="noreferrer">Privacy</a>
          <a href="https://vectorreach.app/terms" style={{fontSize:11,color:'#94a3b8',textDecoration:'none'}} target="_blank" rel="noreferrer">Terms</a>
          <a href="https://vectorreach.app/support" style={{fontSize:11,color:'#94a3b8',textDecoration:'none'}} target="_blank" rel="noreferrer">Support</a>
        </div>
      </div>
    </div>
  )
